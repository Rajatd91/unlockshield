...
Data Sources:
  1. CoinPaprika   — 250+ tokens, prices, volume, market cap, 1h/24h/7d/30d changes
...
Architecture:
  ...
  - CoinPaprika free tier: 10 req/sec (no API key needed)
...
# ═══════════════════════════════════════════════════════════════════════
# SOURCE 1: CoinPaprika — Full Market Data (250+ tokens)
# Free tier: 10 req/sec, no API key needed
# ═══════════════════════════════════════════════════════════════════════

async def coinpaprika_market_full(limit: int = 300) -> List[Dict]:
    """
    Fetch top tokens from CoinPaprika — single request returns all tickers.
    No pagination needed, no rate limiting issues.
    Includes 1h, 24h, 7d, 30d price changes out of the box.
    """
    ...
    for t in all_tokens:
        ...
        mcap = quotes.get("market_cap", 0) or 0
        ...
        enriched.append({
            ...
            "symbol": symbol,
            "name": t.get("name", ""),
            "rank": t.get("rank", 0) or 0,
            "price": price,
            "market_cap": mcap,
            "volume_24h": vol,
            "change_1h": round(quotes.get("percent_change_1h", 0) or 0, 2),
            "change_24h": round(quotes.get("percent_change_24h", 0) or 0, 2),
            "change_7d": round(quotes.get("percent_change_7d", 0) or 0, 2),
            "change_30d": round(quotes.get("percent_change_30d", 0) or 0, 2),
            "volume_to_mcap": round((vol / mcap) * 100, 2) if mcap > 0 else 0,
            "ath": ath,
            "ath_change_pct": round(((price - ath) / ath) * 100, 1) if ath > 0 else 0,
            "sparkline_7d": [],  # CoinPaprika doesn't include sparklines; computed separately if needed
            "sector": classify_sector(symbol),
            "image": "",  # CoinPaprika logos available at /coins/{id}/logo but not needed for dashboard
        })
...
async def coinpaprika_global() -> Dict:
    """Global crypto market statistics from CoinPaprika"""
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.get("https://api.coinpaprika.com/v1/global")
            if resp.status_code == 200:
                data = resp.json()
                result = {
                    "total_market_cap": data.get("market_cap_usd", 0) or 0,
                    "total_volume_24h": data.get("volume_24h_usd", 0) or 0,
                    "btc_dominance": round(data.get("bitcoin_dominance_percentage", 0) or 0, 2),
                    "eth_dominance": round(100 - (data.get("bitcoin_dominance_percentage", 0) or 0) - 60, 2),  # Approximate
                    "active_cryptocurrencies": data.get("cryptocurrencies_number", 0) or 0,
                    "market_cap_change_24h": round(data.get("market_cap_change_24h", 0) or 0, 2),
                    "defi_market_cap": (data.get("market_cap_usd", 0) or 0) * 0.04,
                }
...
async def coinpaprika_prices_batch(symbols_or_ids: List[str]) -> Dict[str, float]:
    """
    Batch price fetch using CoinPaprika tickers endpoint.
    Single request gets all prices — no per-token API calls needed.
    """
    from app.services.market_data import COINPAPRIKA_IDS

    # Build lookup: symbol -> coinpaprika_id
    wanted = {}
    for s in symbols_or_ids:
        s_upper = s.upper()
        cp_id = COINPAPRIKA_IDS.get(s_upper)
        if cp_id:
            wanted[cp_id] = s_upper

    if not wanted:
        return {}

    cache_key = f"prices_{'_'.join(sorted(wanted.keys())[:10])}"
    ...
    try:
        ...
        # Build id -> price map from response
        id_price_map = {}
        for t in data:
            id_price_map[t.get("id", "")] = t.get("quotes", {}).get("USD", {}).get("price", 0) or 0

        prices = {}
        for cp_id, symbol in wanted.items():
            prices[symbol] = id_price_map.get(cp_id, 0)
