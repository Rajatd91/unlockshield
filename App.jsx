  ...
  return(
    <div className="chart-wrap">
      ...
      <svg width="100%" height="180" viewBox="0 0 700 180" preserveAspectRatio="xMidYMid meet">
        <line x1="30" y1="140" x2="680" y2="140" stroke="#e0e8e3" strokeWidth="2"/>
        {(()=>{
          const sorted=unlocks.slice(0,15).map((u,i)=>({...u,_i:i,_d:daysUntil(u.unlock_date)})).sort((a,b)=>a._d-b._d)
          const used=[]
          return sorted.map((u)=>{
            let x=30+Math.min(Math.max(u._d,0),90)/90*650
            // Avoid overlap: nudge if too close to a previous bar
            for(const px of used){if(Math.abs(x-px)<28)x=px+28}
            used.push(x)
            x=Math.min(x,670) // keep in bounds
            ...
            return(
              <g key={u._i} style={{cursor:'pointer'}} onClick={()=>onSelect&&onSelect(u)}>
                <rect x={x-10} y={140-h} width="20" height={h} rx="4" fill={risk} opacity=".75" onMouseEnter={e=>e.target.style.opacity='1'} onMouseLeave={e=>e.target.style.opacity='.75'}/>
                <text x={x} y={155} textAnchor="middle" fontSize="8" fill="#6b8077" fontWeight="600">{u.token_symbol}</text>
                <text x={x} y={133-h} textAnchor="middle" fontSize="8" fill={risk} fontWeight="700">{u.total_supply_percent}%</text>
              </g>
            )
          })
        })()}
        <text x="30" y="172" fontSize="9" fill="#9ca3af">Today</text>
        <text x="680" y="172" fontSize="9" fill="#9ca3af" textAnchor="end">90 days</text>
      </svg>
    </div>
  )
}
...
function App() {
  ...
  const runStressTest = async () => {
    ...
    try {
      ...
      if(!res.ok) { const err = await res.json().catch(()=>({})); toast('Stress Test Failed', err.detail || `Server error ${res.status} — check backend terminal`, 'r'); setStressLoading(false); return }
      const data = await res.json()
      if(!data?.scenarios?.base_case) { toast('Stress Test Error', 'Invalid response — backend may be missing numpy. Run: pip install numpy', 'r'); setStressLoading(false); return }
      setStressResult(data)
      toast('Stress Test Complete',`${stressToken}: VaR(95%)=${data.scenarios.base_case.var_95}%`,'g')
    } catch(e) { toast('Stress Test Error', `Connection failed: ${e.message}. Is the backend running?`, 'r') }
  ...
  const createPrediction = async () => {
    ...
    try {
      ...
      if(!res.ok) { const err = await res.json().catch(()=>({})); toast('Prediction Failed', err.detail || `Server error ${res.status}`, 'r'); setPredLoading(false); return }
      const data = await res.json()
      if(!data?.predicted_impact && data?.predicted_impact !== 0) { toast('Prediction Error', 'No prediction data returned', 'r'); setPredLoading(false); return }
      toast('Prediction Committed',`${stressToken}: ${data.predicted_impact}% impact, hash: ${(data.commit_hash||'').slice(0,12)}...`,'g')
      fetchPredictions()
    } catch(e) { toast('Prediction Error', `Connection failed: ${e.message}`, 'r') }
        ...
        <div className="fade">
          ...
          {stressResult&&(
            <>
              {/* Risk Overview */}
              <div className="sec">
                <div className="sh"><h2><AlertTriangle size={16} color="var(--red)"/> Risk Metrics — {stressResult.token||stressToken}</h2><span className={`rgm rgm-${(stressResult.regime_detected||'sideways').toLowerCase()}`}>{stressResult.regime_detected||'—'} ({((stressResult.regime_confidence||0)*100).toFixed(0)}%)</span></div>
                <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:10,marginBottom:14}}>
                  {(()=>{const b=stressResult.scenarios?.base_case||{};return[
                    {l:'VaR (95%)',v:b.var_95!=null?`${b.var_95}%`:'N/A',c:(b.var_95||0)<-15?'var(--red)':'var(--yellow)'},
                    {l:'CVaR (95%)',v:b.cvar_95!=null?`${b.cvar_95}%`:'N/A',c:'var(--red)'},
                    {l:'P(Loss>10%)',v:b.prob_loss_gt_10pct!=null?`${(b.prob_loss_gt_10pct*100).toFixed(1)}%`:'N/A',c:(b.prob_loss_gt_10pct||0)>0.5?'var(--red)':'var(--yellow)'},
                    {l:'Max Drawdown',v:b.max_drawdown_worst!=null?`${b.max_drawdown_worst}%`:'N/A',c:'var(--red)'},
                    {l:'Mean Return',v:b.mean_return!=null?`${b.mean_return}%`:'N/A',c:(b.mean_return||0)<0?'var(--red)':'var(--green)'},
                  ]})().map((m,i)=>(
              ...
              <div className="sec">
                ...
                <div className="tw"><table><thead><tr><th>Scenario</th><th>VaR(95%)</th><th>CVaR(95%)</th><th>P(Loss>10%)</th><th>Mean Return</th><th>Skewness</th><th>Kurtosis</th></tr></thead><tbody>
                  {Object.entries(stressResult.scenarios||{}).map(([k,v])=>{if(!v)return null;return(
                    <tr key={k}><td style={{fontWeight:600}}>{k.replace(/_/g,' ').replace(/\b\w/g,c=>c.toUpperCase())}</td>
                    <td style={{color:(v.var_95||0)<-15?'var(--red)':'var(--yellow)',fontWeight:700}}>{v.var_95??'—'}%</td>
                    <td style={{color:'var(--red)',fontWeight:700}}>{v.cvar_95??'—'}%</td>
                    <td>{v.prob_loss_gt_10pct!=null?(v.prob_loss_gt_10pct*100).toFixed(1):'—'}%</td>
                    <td style={{color:(v.mean_return||0)<0?'var(--red)':'var(--green)',fontWeight:600}}>{v.mean_return??'—'}%</td>
                    <td>{v.skewness??'—'}</td>
                    <td>{v.kurtosis??'—'}</td></tr>
                  )})}
                </tbody></table></div>
